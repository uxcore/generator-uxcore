...
module.exports = class extends Generator {
  app() {
    ...
    this.ComponentName = _.upperFirst(_.camelCase(pkg.name.split('-').slice(1).join('-')));
    tryIfNotExist('tests', function () {
      me.fs.copyTpl('tests/ComponentName.spec.js', 'tests/' + me.ComponentName + '.spec.jsx', this);
      me.fs.copy('tests/index.js', 'tests/index.js');
    });
    tryIfNotExist('.eslintrc.json', function () {
      me.fs.copy('_eslintrc.json', '.eslintrc.json');
    });
    this.fs.copy('_travis.yml', '.travis.yml');
    this.fs.copy('_gitignore', '.gitignore');
    this.fs.copy('_npmignore', '.npmignore');
  }
}
